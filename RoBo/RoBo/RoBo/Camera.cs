...
namespace RoBo
{
    ...
    public interface ICamera2D
    {
        ...
        /// </returns>
        bool IsInView(Vector2 position, Rectangle rec);
    }
    ...
    public class Camera2D : GameComponent, ICamera2D
    {
        ...
        /// </summary>
        /// <param name="position">The position.</param>
        /// <param name="rec">The texture.</param>
        /// <returns>
        ///     <c>true</c> if [is in view] [the specified position]; otherwise, <c>false</c>.
        /// </returns>
        public bool IsInView(Vector2 position, Rectangle rec)
        {
            // If the object is not within the horizontal bounds of the screen

            if ((position.X + rec.Width) < (Position.X - Origin.X) || (position.X) > (Position.X + Origin.X))
                return false;

            // If the object is not within the vertical bounds of the screen
            if ((position.Y + rec.Height) < (Position.Y - Origin.Y) || (position.Y) > (Position.Y + Origin.Y))
                return false;
